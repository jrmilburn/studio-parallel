...
export default function Menu({ open, setOpen } : { open : boolean, setOpen : React.Dispatch<React.SetStateAction<boolean>>}) {
    ...
    return (
        <div className={`fixed inset-0 bg-black flex justify-between z-[100] transition-all duration-1000 overflow-hidden ${open ? "translate-y-[0]" : "translate-y-[-100%]"} ${open === false && "delay-800"}`}>
            ...
            <div className="flex flex-col justify-between text-white">
                ...
                <ul className="flex flex-col">
                    <Link className="text-[5vw]" href="/" onClick={() => setOpen(false)}><AnimateIn shown={open} delay={open === false ? "delay-600" : "delay-500"}><HoverAnimate>HOME</HoverAnimate></AnimateIn></Link>
                    <Link className="text-[5vw]" href="/services" onClick={() => setOpen(false)}><AnimateIn shown={open} delay={open === false ? "delay-500" : "delay-600"}><HoverAnimate>SERVICES</HoverAnimate></AnimateIn></Link>
                    <Link className="text-[5vw]" href="/works" onClick={() => setOpen(false)}><AnimateIn shown={open} delay={open === false ? "delay-400" : "delay-700"}><HoverAnimate>WORKS</HoverAnimate></AnimateIn></Link>
